...
// A Tigr bitmap.
typedef struct Tigr {
    int flags;          // flags for the bitmap
...
//
Tigr *tigrMainWindow(int w, int h, const char *title, int flags);

// Creates an empty off-screen bitmap.
Tigr *tigrBitmap(int w, int h);

// Creates an empty off-screen bitmap with gl texture backing to support later sreen draws
